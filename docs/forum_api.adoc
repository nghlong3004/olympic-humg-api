...
GET /api/forum/thread?type=QUESTION&keyword=limit
----
...
marks `is_active = false` for the thread.
Associated `forum_post` entries may remain, or be hidden according to backend logic.
