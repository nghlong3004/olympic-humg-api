...
| `firstName`     | string (1..25)  | yes      | Tên, không rỗng
| `lastName`      | string (1..25)  | yes      | Họ, không rỗng
| `email`         | email           | yes      | Duy nhất trong hệ thống
| `password`      | string (>= 6)   | yes      | Mật khẩu
| `gender`        | enum            | yes      | `MALE`, `FEMALE`, `OTHER`
| `birthday`      | date            | yes      | Format `YYYY-MM-DD`
| `phone`         | string          | no       | Số điện thoại
| `universityName`| string (<= 50)  | no       | Tên trường
| `facultyName`   | string (<= 50)  | no       | Tên khoa
|===
